...
export function boardReducer(state = initialState, action = {}) {
  ...
  switch (action.type) {
    ...
    case SET_PREV_BOARD: {
      const prevBoard =
        action.prevBoard === undefined ? null : action.prevBoard;
    ...
    case REMOVE_BOARD: {
      ...
      if (!boardId) break;
      const lastRemovedBoard =
        safeBoards.find((b) => b && b._id === boardId) || null;
    ...
    case UPDATE_BOARD: {
      ...
      const boards = safeBoards.map((b) =>
        b && b._id === updated._id ? updated : b,
      );
      const board =
        state.board && state.board._id === updated._id ? updated : state.board;
    ...
    case SET_FILTERBY: {
      const filterBy =
        action.filterBy === undefined ? state.filterBy : action.filterBy;
