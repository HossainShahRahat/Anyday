...
export async function updateBoard(board, data, type) {
  try {
    const boardToUpdate = await boardService.updateBoardService(
      board,
      data,
      type,
    );
...
export async function updateGroup(board, data, type) {
  try {
    const boardToUpdate = await boardService.updateGroupsService(
      board,
      data,
      type,
    );
...
export async function updateTask(board, data, type) {
  try {
    const boardToUpdate = await boardService.updateTaskService(
      board,
      data,
      type,
    );
...
export function handleOnDragEnd(res, data) {
  ...
  switch (type) {
    case "task-list":
      if (source.droppableId !== destination.droppableId) {
        const sourceGroup = boardCopy.groups.find(
          (group) => group.id === draggedFromId,
        );
        const destGroup = boardCopy.groups.find(
          (group) => group.id === draggedToId,
        );
        const sourceTasks =
          sourceGroup && sourceGroup.tasks ? sourceGroup.tasks : [];
        const destTasks = destGroup && destGroup.tasks ? destGroup.tasks : [];
        const [removed] = sourceTasks.splice(source.index, 1);
        destTasks.splice(destination.index, 0, removed);
      } else {
        const group = boardCopy.groups.find(
          (group) => group.id === draggedFromId,
        );
    ...
    case "task-card":
      if (source.droppableId !== destination.droppableId) {
        const destStatus = boardCopy.statuses.find(
          (status) => status.id === draggedToId,
        );
        const groupToUpdate = boardCopy.groups.find((group) =>
          (group.tasks || []).find((task) => task.id === draggableId),
        );
        const taskToUpdate =
          groupToUpdate && groupToUpdate.tasks
            ? groupToUpdate.tasks.find((task) => task.id === draggableId)
            : null;
        if (taskToUpdate && destStatus) taskToUpdate.status = destStatus.label;
      } else {
        // reorder statuses array safely
        const statusIdx = boardCopy.statuses.findIndex(
          (s) => s.id === draggedFromId,
        );
        if (statusIdx >= 0) {
