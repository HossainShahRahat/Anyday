...
export function TaskPreview({ task, board, group, openModal, provided, snapshot, setIsDndModeDisabled, setIsCheckedShow }) {
    ...
    // Defensive shallow copies - do not mutate props
    const safeBoard = board || { cmpsOrder: [] }
    const safeGroup = group || { id: 'group-unknown', style: '#808080' }
    const safeTask = task || { id: 'task-unknown', title: '', isChecked: false }
    ...
    function handleChange({ target }) {
        ...
        const data = { taskId: safeTask.id, groupId: safeGroup.id, labelPick: value }
        let updateType = type === 'number' ? UPDATE_TASK_LABEL_NUMBER : UPDATE_TASK_LABEL_TEXT

        updateTask(safeBoard, data, updateType)
    }

    function handleChangeTaskChecked({ target }, taskId) {

        updateTask(safeBoard, { taskId, checked: target.checked, groupId: safeGroup.id }, UPDATE_TASK_CHECKED)
        ...
        >
            ...
            >
                ...
                >
                    ...
                    <button className="msg-btn" style={(task.comments && task.comments.length) ? { paddingRight: '5px' } : { paddingRight: '19px', paddingLeft: '20px' }}>

                        {(!(task.comments && task.comments.length)) && <Icon SVG="AddUpdate" iconType={Icon.type.SVG} icon={AddUpdate} iconLabel="Task Details" iconSize={24} />}
                        {(task.comments && task.comments.length) && <div className="storybook-counter_position">
