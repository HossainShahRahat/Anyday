...
export function DynamicSummaryCmp({ cmp, board, group }) {
    const safeBoard = board || { statuses: [], labelStatuses: [], priorities: [] }
        ...
        switch (cmp) {
            case STATUS_PICKER: {
                ...
                for (const key in GroupStatuses) {
                    const found = (localBoard.statuses || []).find(status => status && status.label === key)
                    if (found) {
            ...
            case LABEL_STATUS_PICKER: {
                ...
                for (const key in GroupLabelsStatuses) {
                    const found = (localBoard.labelStatuses || []).find(status => status && status.label === key)
            ...
            case PRIORITY_PICKER: {
                ...
                for (const key in GroupPriorities) {
                    const found = (localBoard.priorities || []).find(status => status && status.label === key)
                ...
                <div className="status-sum-container">
                    {
                        (gruoupSummaryCalc(cmp) || []).map((statusSum, idx) => {
                            const width = taskCount > 0 ? `${(statusSum.value / taskCount) * 100}%` : '0%'
                ...
                <div className="label-status-picker-sum">
                    {
                        (gruoupSummaryCalc(cmp) || []).map((labelStatusSum, idx) => {
                            const width = taskCount > 0 ? `${(labelStatusSum.value / taskCount) * 100}%` : '0%'
                            return <div key={idx} className="status-sum"
                                style={{
                ...
                <div className="priority-picker-sum">
                    {
                        (gruoupSummaryCalc(cmp) || []).map((prioritySum, idx) => {
                            const width = taskCount > 0 ? `${(prioritySum.value / taskCount) * 100}%` : '0%'
                            return <div key={idx} className="status-sum"
                                style={{
