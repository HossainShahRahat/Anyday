...
import { updateTask, addTaskComment } from "../store/board.actions";
...
import {
  ...
  PIN_TASK_COMMENT,
  UNPIN_TASK_COMMENT,
} from "../services/board.service.local.js";
...
export function TaskDetails({
  ...
  // matchMedia values are computed in the effect where needed

  // run once on mount to set initial sizes; mediaSizeOptimize references globals and is stable
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => {
    // Determine initial comment panel size based on viewport
  ...
  // keep newTitle in sync with incoming task prop
  useEffect(() => {
    setNewTitle(task?.title || "");
  }, [task]);

  // Add a new comment to a task.
  // Defensive: bail out if required data is missing. Do not mutate props.
  async function onSubmitNewComment(ev) {
    ev.preventDefault();
    if (!newCommentTxt || !task || !group || !board) return;
    // normalize arrays in task to avoid service-side crashes
    task.comments = Array.isArray(task.comments) ? task.comments : [];
    task.pinnedComments = Array.isArray(task.pinnedComments)
      ? task.pinnedComments
      : [];
  ...
  function onDeleteComment(comment, isPinned = false) {
    if (!comment || !task || !group || !board) return;
    let taskChanges = {
  ...
  function getAvatarImg(comment) {
    if (comment.byMember?.imgUrl) {
      return (
        <img
          className="task-details-by-user-img"
      ...
      return (
        <div className="task-details-by-user-img">
          {comment.byMember.fullname.charAt(0).toUpperCase()}
        </div>
      );
    } else {
  ...
  function renderComments(taskComments, isPinned = false) {
    // Defensive rendering: normalize to array and only render when non-empty
    const comments = Array.isArray(taskComments) ? taskComments : [];
    if (!comments.length) return null;
    return (
      <section>
        {comments.map((comment) => (
          <div key={comment.id} className="task-details-task-comment">
            {isPinned && (
              <div className="pinnedComment">
                <Icon
                  className="task-details-header-time-icon"
                  iconType={Icon.type.SVG}
                  icon={Pin}
                  iconLabel="my svg icon"
                  iconSize={14}
                />
                Pinned
              </div>
            ...
            <div className="task-details-header">
              <div className="task-details-by-user">
                ...
                <h1 className="task-details-by-user-name">
                  {comment.byMember?.fullname || "Guest User"}
                </h1>
              </div>
              ...
              <div className="task-details-header-tools">
                <Icon
                  ...
                  iconSize={14}
                />
                <div className="task-details-created-at">
                  {utilService.time_ago(comment.createdAt)}
                </div>
                <MenuButton className="task-details-menu-btn">
                  <Menu id="task-details-menu" size="medium">
                    <MenuItem
                      onClick={() => {
                        isPinned
                          ? onUnpinFromTop(comment)
                          : onPinToTop(comment);
                      }}
                      icon={Pin}
            ...
            <p
              className="task-comment-inner-text"
              dangerouslySetInnerHTML={{ __html: comment.txt || "" }}
            ></p>
            {comment.imgUrl && comment.imgUrl !== "" ? (
  ...
  return (
    <section
      ...
      style={{ width: `${isOpenDetails ? 100 : 1}vw` }}
    >
      <div className="task-main" style={{ width: `${taskCommentsSize}%` }}>
        ...
        <TabList className="task-main-nav">
          ...
          <Tab onClick={(ev) => setTabList(ev)}>Activity Log</Tab>
        </TabList>
        ...
        {tabList === 0 ? (
          <div>
            {!isAddComment && (
              <div className="task-details-open-input-btn-container">
                <button
                  onClick={() => setAddComment(!isAddComment)}
                  className="task-details-open-input-btn"
                >
                  Write an update...
                </button>
              </div>
              ...
              <form className="task-details-form" onSubmit={onSubmitNewComment}>
                <div className="task-details-textbox-container">
                    ...
                    <div className="emoji-picker">
                      {emojis.map((emoji) => (
                        <span
                          key={emoji}
                          className="emoji"
                          onClick={() => setComment(newCommentTxt + emoji)}
                        >
                          {emoji}
                        </span>
                      ))}
                  ...
                  <div className="task-details-input-footer">
                    <span
                      onClick={handleClick}
                      className="task-details-input-upload"
                    >
                      <Icon
                        className="task-details-header-time-icon"
                        iconType={Icon.type.SVG}
                    ...
                    <span
                      ...
                      className="task-details-input-upload emoji"
                    >
                      <Icon
                        className="task-details-header-time-icon"
                        iconType={Icon.type.SVG}
                    ...
                    <button className="btn task-details-input-btn">
                      Update
                    </button>
                  </div>
                </div>
                {imgSrc && (
                  <span className="task-details-img-preview-container">
                    <img
                      className="task-details-img-preview"
                      src={imgSrc}
            ...
            {(Array.isArray(task?.pinnedComments) &&
              task.pinnedComments.length) ||
            (Array.isArray(task?.comments) && task.comments.length) ? (
              ...
              <section>
                ...
                <p className="details-p">
                  <span className="details-p-header">
                    No updates yet for this item
                  </span>
                  <span className="details-p-txt">
        ...
        <div
          className="slide-panel-resizer"
          draggable="true"
          onDrag={dragstart}
          onMouseDown={getInitX}
        >
          <Icon
            className="task-details-header-time-icon"
            iconType={Icon.type.SVG}
      ...
      <div
        ...
        style={{ width: `${100 - taskCommentsSize}%` }}
        onClick={() => {
          setIsOpenDetails(!isOpenDetails);
          setIsDndModeDisabled(false);
        }}
