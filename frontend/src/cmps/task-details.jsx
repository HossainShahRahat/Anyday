...
import { CHANGE_TASK_TITLE, DELETE_TASK_COMMENT, PIN_TASK_COMMENT, UNPIN_TASK_COMMENT } from '../services/board.service.local.js';
...
export function TaskDetails({ task, isOpenDetails, setIsOpenDetails, board, group, setIsDndModeDisabled }) {
    ...
    async function onSubmitNewComment(ev) {
        ...
        task.pinnedComments = Array.isArray(task.pinnedComments) ? task.pinnedComments : []
        await addTaskComment(board, group.id, task.id, newCommentTxt, imgSrc, loggedInUser)
        setComment('')
        setImg('')
    ...
    function renderComments(taskComments, isPinned = false) {
        ...
        return <section>
                {comments.map(comment => <div
                    ...
                    {comment.imgUrl && comment.imgUrl !== '' ? <img src={`${comment.imgUrl}`} alt="" /> : null}
            ...
            {tabList === 0 ? <div>
                ...
                {isAddComment &&
                    ...
                    <form className='task-details-form' onSubmit={onSubmitNewComment}>
                        ...
                        {imgSrc && <span className="task-details-img-preview-container" ><img className="task-details-img-preview" src={imgSrc} alt="uploaded preview" /><span>Uploaded</span></span>}
                ...
                {((Array.isArray(task?.pinnedComments) && task.pinnedComments.length) || (Array.isArray(task?.comments) && task.comments.length)) ? '' : <section>
                    <div className='details-img-container'><img className="details-img" src="https://cdn.monday.com/images/pulse-page-empty-state.svg" alt="no updates" /></div>
