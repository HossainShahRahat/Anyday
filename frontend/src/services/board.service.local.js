        ...
        if (Array.isArray(filterBy.label) && filterBy.label.length) {
            let groupsToSet = filteredGroups.filter((group, idx) => {
                ...
                const tasks = (group.tasks || []).filter(task => {
                    return (filterBy.label.includes(task.status) || filterBy.label.includes(task.priority) || filterBy.label.includes(task.labelStatus))
                })
                if (tasks.length) {
                    filteredGroups[idx] = { ...group, tasks }
                    return true
                ...
                board.groups.forEach((group, groupIdx) => {
                    ...
                    group.tasks.forEach((task, taskIdx) => {
                        if (dTask.id === task.id) {
                            ...
                            if (groupToUpdate >= 0) {
                                if (!Array.isArray(board.groups[groupToUpdate].tasks)) board.groups[groupToUpdate].tasks = []
...
async function updateTaskService(board, data, type) {
    ...
    if ((type === UPDATE_TASK_MEMBERS || type === UPDATE_TASK_LABEL_STATUS || type === UPDATE_TASK_STATUS || type === UPDATE_TASK_PRIORITY)) {
    ...
    switch (type) {
        ...
        case DELETE_TASK_COMMENT:
            currTask = board.groups[groupIdx] && board.groups[groupIdx].tasks && board.groups[groupIdx].tasks[taskIdx]
            ...
            if (!data.isPinned) {
                ...
                if (deleteCommentIdx >= 0) currTask.comments.splice(deleteCommentIdx, 1)
            } else {
                ...
                if (deleteCommentIdx >= 0) currTask.pinnedComments.splice(deleteCommentIdx, 1)
            }
            return board
        ...
        case UPDATE_TASK_STATUS:
            ...
            if (!Array.isArray(board.groups[groupIdx].tasks[taskIdx].activity)) board.groups[groupIdx].tasks[taskIdx].activity = []
