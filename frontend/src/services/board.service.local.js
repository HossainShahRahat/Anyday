/**
 * board.service.local.js
 * Local service wrapper for board operations used by UI and store actions.
 *
 * Stability guarantees provided here:
 * - All returned board objects have a predictable shape for UI code: arrays for `groups`,
 *   each group's `tasks`, and task-level arrays for `comments`/`pinnedComments`/`members`.
 * - All public update functions are defensive: they validate indices and bail safely
 *   when a referenced group/task doesn't exist.
 * - This file never throws unexpectedly for missing optional fields; caller errors are
 *   logged and passed up only when appropriate.
 *
 * When adding new mutation types, follow the pattern:
 * - compute safe groupIdx/taskIdx using helpers
 * - guard before mutating arrays
 * - return the modified board or the original board if an operation couldn't be applied
 */
...
// --- Production-level helpers & contracts ---
// See helper functions below for safe index lookup and defensive behavior.

// safeIndex helpers: return -1 when not found so callers can bail out cleanly
function findGroupIndex(board, groupId) {
  if (!board || !Array.isArray(board.groups)) return -1;
  return board.groups.findIndex((g) => g && g.id === groupId);
}

function findTaskIndex(board, groupIdx, taskId) {
  if (!board || !Array.isArray(board.groups) || groupIdx < 0) return -1;
  const group = board.groups[groupIdx];
  if (!group || !Array.isArray(group.tasks)) return -1;
  return group.tasks.findIndex((t) => t && t.id === taskId);
}
...
async function getById(boardId, filterBy = getDefaultFilter()) {
  try {
    ...
    if (filterBy.title) {
      ...
      let groupsToSet = filteredGroups.filter((group, idx) => {
        ...
        let tasks = (group.tasks || []).filter((task) =>
          (task.title || "").match(regex),
        );
        if (tasks.length) {
          // clone group and set filtered tasks so downstream code expects consistent structure
          filteredGroups[idx] = { ...group, tasks };
          return true;
        }
        return false;
      });
    ...
    if (Array.isArray(filterBy.label) && filterBy.label.length) {
      let groupsToSet = filteredGroups.filter((group, idx) => {
        if (!group) return false;
        const tasks = (group.tasks || []).filter((task) => {
          return (
            filterBy.label.includes(task.status) ||
            filterBy.label.includes(task.priority) ||
            filterBy.label.includes(task.labelStatus)
          );
        });
        if (tasks.length) {
          filteredGroups[idx] = { ...group, tasks };
...
async function updateGroupsService(board, data, type) {
  ...
  switch (type) {
    case CHANGE_GROUP_TITLE:
      groupIdx = board.groups.findIndex(
        (currGroup) => currGroup.id === data.id,
      );
      board.groups.splice(groupIdx, 1, data);
      return board;
    case CHANGE_GROUP_COLOR:
      groupToUpdate = board.groups.find(
        (currGroup) => currGroup.id === data.group.id,
      );
      groupToUpdate.style = data.color;
      return board;
    case ADD_GROUP_FROM_HEADER:
      // defensive: ensure groups array exists before unshift
    ...
    case ADD_GROUP:
      ...
      if (groupIdx >= 0) board.groups.splice(groupIdx + 1, 0, getEmptyGroup());
      else board.groups.push(getEmptyGroup());
      return board;
    case DUPLICATE_GROUP:
      ...
      if (groupIdx >= 0) board.groups.splice(groupIdx, 0, data);
      else board.groups.push(data);
      return board;
    case ADD_GROUP_TASK:
    ...
    case ON_DRAG_TASK:
      ...
      if (groupIdx >= 0) board.groups.splice(groupIdx, 1, data);
      else board.groups.push(data);
      return board;
    ...
    case REMOVE_TASKS_FROM_GROUP:
      groupToUpdate = board.groups.map((group, idx) => {
        ...
        // only assign tasks when provided data is an array
        group.tasks = Array.isArray(data[idx]) ? data[idx] : [];
        return group.tasks;
      });
    ...
    case DUPLICATE_CHECKED_TASKS:
      groupToUpdate = board.groups.forEach((group, Groupidx) => {
        ...
        group.tasks.forEach((task, taskIdx) => {
          if (Array.isArray(data[Groupidx]) && data[Groupidx][taskIdx]) {
            // insert duplicate safely
            group.tasks.splice(taskIdx, 0, data[Groupidx][taskIdx]);
          }
    ...
    case MOVE_TASK_TO_GROUP:
      groupToUpdate = board.groups.findIndex(
        ...
        board.groups.forEach((group, groupIdx) => {
          ...
          group.tasks.forEach((task, taskIdx) => {
            if (dTask.id === task.id) {
              ...
              if (groupToUpdate >= 0) {
                if (!Array.isArray(board.groups[groupToUpdate].tasks))
                  board.groups[groupToUpdate].tasks = [];
...
async function updateTaskService(board, data, type) {
  ...
  if (data) {
    groupIdx = findGroupIndex(board, data.groupId);
    taskIdx = findTaskIndex(board, groupIdx, data.taskId);
  }
  // Ensure activity array exists before pushing activities
  if (
    type === UPDATE_TASK_MEMBERS ||
    type === UPDATE_TASK_LABEL_STATUS ||
    type === UPDATE_TASK_STATUS ||
    type === UPDATE_TASK_PRIORITY
  ) {
    if (groupIdx >= 0 && taskIdx >= 0) {
  ...
  switch (type) {
    case DELETE_TASK:
      board.groups[groupIdx].tasks = board.groups[groupIdx].tasks.filter(
    ...
    case ADD_TASK_FROM_HEADER:
      // defensive: ensure first group and its tasks array exist
      if (!Array.isArray(board.groups)) board.groups = [getEmptyGroup()];
      if (!Array.isArray(board.groups[0].tasks)) board.groups[0].tasks = [];
    ...
    case ADD_TASK_COMMENT:
      // Defensive: ensure task/comment arrays exist before mutating
      if (groupIdx < 0 || taskIdx < 0) return board;
      if (!Array.isArray(board.groups[groupIdx].tasks[taskIdx].comments)) {
        board.groups[groupIdx].tasks[taskIdx].comments = [];
      }
      // normalized: safe to unshift
      board.groups[groupIdx].tasks[taskIdx].comments.unshift(data.comment);
      return board;
    case DELETE_TASK_COMMENT:
      currTask =
        board.groups[groupIdx] &&
        board.groups[groupIdx].tasks &&
        board.groups[groupIdx].tasks[taskIdx];
      if (!currTask) return board;
      if (!data.isPinned) {
        if (!Array.isArray(currTask.comments)) currTask.comments = [];
        let deleteCommentIdx = currTask.comments.findIndex(
          (currComment) => currComment.id === data.commentIdx,
        );
        if (deleteCommentIdx >= 0)
          currTask.comments.splice(deleteCommentIdx, 1);
      } else {
        if (!Array.isArray(currTask.pinnedComments))
          currTask.pinnedComments = [];
        let deleteCommentIdx = currTask.pinnedComments.findIndex(
          (currComment) => currComment.id === data.commentIdx,
        );
        if (deleteCommentIdx >= 0)
    ...
    case PIN_TASK_COMMENT:
      currTask =
        ...
        board.groups[groupIdx].tasks[taskIdx];
      if (!currTask) return board;
      let PinCommentIdx = (currTask.comments || []).findIndex(
        (currComment) => currComment.id === data.commentIdx,
      );
      if (PinCommentIdx < 0) return board;
      if (!Array.isArray(currTask.pinnedComments)) currTask.pinnedComments = [];
      // ensure comments array exists before moving
      if (!Array.isArray(currTask.comments)) currTask.comments = [];
      currTask.pinnedComments.unshift(currTask.comments[PinCommentIdx]);
    ...
    case UNPIN_TASK_COMMENT:
      currTask =
        ...
        board.groups[groupIdx].tasks[taskIdx];
      if (!currTask) return board;
      let UnpinCommentIdx = (currTask.pinnedComments || []).findIndex(
        (currComment) => currComment.id === data.commentIdx,
      );
      if (UnpinCommentIdx < 0) return board;
      if (!Array.isArray(currTask.comments)) currTask.comments = [];
      // ensure pinnedComments exists
      if (!Array.isArray(currTask.pinnedComments)) currTask.pinnedComments = [];
      currTask.comments.unshift(currTask.pinnedComments[UnpinCommentIdx]);
    ...
    case UPDATE_TASK_STATUS:
      ...
      if (!Array.isArray(board.groups[groupIdx].tasks[taskIdx].activity))
        board.groups[groupIdx].tasks[taskIdx].activity = [];
    ...
    case UPDATE_TASK_MEMBERS:
      ...
      if (data.isDelete) {
        ...
        if (!Array.isArray(board.groups[groupIdx].tasks[taskIdx].activity))
          board.groups[groupIdx].tasks[taskIdx].activity = [];
        board.groups[groupIdx].tasks[taskIdx].activity.unshift(activity);
        let memberToDeleteIdx = board.groups[groupIdx].tasks[
          taskIdx
        ].members.findIndex((member) => member._id === data.labelPick._id);
        board.groups[groupIdx].tasks[taskIdx].members.splice(
          memberToDeleteIdx,
          1,
        );
      } else {
        ...
        board.groups[groupIdx].tasks[taskIdx].activity.unshift(activity);
        if (
          board.groups[groupIdx].tasks[taskIdx].members.find(
            (member) => member._id === data.labelPick._id,
          )
        )
          return;
