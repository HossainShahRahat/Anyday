...
export function BoardDetails() {
    ...
    const safeBoard = board || { groups: [], statuses: [], priorities: [], cmpsOrder: [] }
    useEffect(() => {
    ...
    function openModal(ev, data, info) {
        ...
        const labelPos = target ? target.getBoundingClientRect() : { top: 0, left: 0 }
        const boardScrollTop = boardContainer.current ? boardContainer.current.scrollTop : 0
        const boardScrollLeft = boardContainer.current ? boardContainer.current.scrollLeft : 0
        setIsModalOpen(true)
    ...
    if (!board || !board.groups) return <div className="loader"><Loader size={Loader.sizes.LARGE} /></div>
                    ...
                    handleOnDragEnd(res,
                        {
                            ...
                            grouplist: (prevBoard && Array.isArray(prevBoard.groups)) ? prevBoard.groups : [],
                            // defensive: ensure cmpsOrder is an array
                            cmpsOrder: Array.isArray(board?.cmpsOrder) ? board.cmpsOrder : []
                        })}>
