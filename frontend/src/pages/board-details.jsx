...
import {
  ADD_GROUP_FROM_BUTTOM,
  ADD_GROUP_FROM_HEADER,
  ADD_TASK_FROM_HEADER,
  DATE_PICKER,
  DUPLICATE_CHECKED_TASKS,
  LABEL_STATUS_PICKER,
  MEMEBER_PICKER,
  MOVE_TASK_TO_GROUP,
  PRIORITY_PICKER,
  REMOVE_CHECKED_VALUE_GROUPS,
  REMOVE_TASKS_FROM_GROUP,
  STATUS_PICKER,
  UPDATE_TASK_DATE,
...
export function BoardDetails() {
  ...
  const prevBoard = useSelector(
    (storeState) => storeState.boardModule.prevBoard,
  );
  ...
  // Defensive local view: used only for safe rendering (do not replace original `board` when calling service/actions)
  ...
  function checkIfTaskChecked() {
    if (!board || !Array.isArray(board.groups)) return 0;
  ...
  function DynamicInfo(data, info) {
    ...
    switch (info) {
      case STATUS_PICKER:
        return safeBoard.statuses || [];
      case LABEL_STATUS_PICKER:
        return safeBoard.labelStatuses || [];
      case MEMEBER_PICKER:
        return {
  ...
  function openModal(ev, data, info) {
    ...
    const labelPos = target
      ...
      : { top: 0, left: 0 };
    const boardScrollTop = boardContainer.current
      ? boardContainer.current.scrollTop
      : 0;
    const boardScrollLeft = boardContainer.current
      ? boardContainer.current.scrollLeft
      : 0;
    setIsModalOpen(true);
    ...
    return setCmp((prev) => {
      return {
        ...
        pos: {
          top: labelPos.top + boardScrollTop,
          left: labelPos.left + boardScrollLeft,
        },
        type: info,
  ...
  function onMoveTasks(ev, groupId) {
    ...
    updateGroup(
      board,
      { groupId, tasks: checkedTasks.flat(1) },
      MOVE_TASK_TO_GROUP,
    );
  }
  ...
  function onDragStart({ type }) {
    if (type !== "group-list" || !board) return;
    setPrevBoard(board);
    ...
    <section className="board-details">
      ...
      {board && (
        <div ref={boardContainer} className="board-container">
          ...
          <DragDropContext
            ...
            onDragEnd={(res) =>
              handleOnDragEnd(res, {
                board,
                prevBoard,
                // defensive: prevBoard may be null during some transitions
                grouplist:
                  prevBoard && Array.isArray(prevBoard.groups)
                    ? prevBoard.groups
                    : [],
                // defensive: ensure cmpsOrder is an array
                cmpsOrder: Array.isArray(board?.cmpsOrder)
                  ? board.cmpsOrder
                  : [],
              })
            }
              ...
              {(provided) => (
                ...
                >
                  ...
                  <div className="bottom-add-group-btn-container">
                    <button
                      className="btn clean bottom-add-group-btn"
                      onClick={() =>
                        updateGroup(board, null, ADD_GROUP_FROM_BUTTOM)
                      }
                    >
                      <Icon iconType={Icon.type.SVG} icon={Add} iconSize={19} />{" "}
                      Add new group
                    </button>
                  </div>
                </section>
          ...
          <div className="add-group-btn-container"></div>

          {isCheckedShow && (
            <div className="checkbox-modal-wrapper">
                ...
                <div className="btns-container">
                  ...
                  <div className="checknox-delete-btn-container">
                    <button onClick={onDuplicateTasks}>
                      <FontAwesomeIcon className="icon-delete" icon={faCopy} />
                    </button>
                    <span>Duplicate</span>
                  </div>
                  <div className="checknox-delete-btn-container">
                  ...
                  <div className="checknox-move-btn-container">
                    <button onClick={() => setisMoveToShow(!isMoveToShow)}>
                      {" "}
                      <FontAwesomeIcon
              ...
              {isMoveToShow && (
                <div className="move-to-wrapper">
                  <div className="menu-checkbox-controls">
                    <div className="choose-group-txt">Choose group</div>
                    <div
                      className="choose-back"
                      onClick={() => setisMoveToShow(false)}
                    >
                      Back
                    </div>
                  </div>
      ...
      <div className="btn-add-mobile-container">
        ...
        >
          <Icon
            className="icon-mobile-add"
            ignoreFocusStyle={true}
            style={{ rotate: isMobileMenuOpen ? "405deg" : "0deg" }}
            iconType={Icon.type.SVG}
            icon={Add}
            iconSize={22}
          />
