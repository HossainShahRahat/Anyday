...
const {
  boardService,
  ADD_TASK_COMMENT,
} = require("../services/board.service.local.js");
...
describe("board.service.local smoke", () => {
  test("updateTaskService - add comment (mocked http)", async () => {
    ...
    board.groups[0].tasks =
      board.groups[0].tasks && board.groups[0].tasks.length
        ? board.groups[0].tasks
        : [{ id: "t1", title: "T1", comments: [] }];
    ...
    const comment = {
      id: "c1",
      txt: "Hello",
      byMember: { fullname: "Test User" },
      createdAt: Date.now(),
    };
    const result = await boardService.updateTaskService(
      { _id: "board_test" },
      { groupId, taskId, comment },
      ADD_TASK_COMMENT,
    );

    const updatedTask = result.groups
      .find((g) => g.id === groupId)
